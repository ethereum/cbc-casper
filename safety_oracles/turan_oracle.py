...
class TuranOracle:
    ...
    def find_biggest_clique(self):
        ...
        # for reasoning behind this, see https://gist.github.com/naterush/d867fe35ecb63c42e6c1f053e2ff33a8
        min_size_max_clique = int(math.ceil(float(math.pow(s.NUM_VALIDATORS, 2)) / \
                              float(math.pow(s.NUM_VALIDATORS, 2) - 2 * num_edges)))

        sorted_validators = sorted([(v, s.WEIGHTS[v]) for v in s.WEIGHTS], key=lambda tup: tup[1])

        max_clique = set()

        for i in range(0, min_size_max_clique):
            (val, _) = sorted_validators[i]
            max_clique.add(val)
